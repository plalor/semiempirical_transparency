...
### Generating files with targets

lmbdaRange = np.linspace(0, 300, 4)[1:]
zRange = np.arange(1, 99)[::25]
num_particles = 1e5
...
for Z in zRange:
    ...
    for lmbda in lmbdaRange:
        filename = "lmbda=%d,Z=%d.gdml" % (lmbda, Z)
...
<!-- OUTPUT FILTERS.  What data/entries do you want to keep? -->
  ...
  <define>
    ...
    <constant name="EventsToRun" value="{num_particles}"/>
